...
class Lv2ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //curLevel = 2
        resetGame()
        initGame(numberOfCards: 14, lblCards:lblCollectionCards,lblTap:lblTap)
        arrButtons = [btn0,btn1,btn2,btn3,btn4,btn5,btn6,btn7,btn8,btn9,btn10,btn11,btn12,btn13]
        limitTime = 35
        countDown = limitTime
    ...
    @IBAction func btnATapone(_ sender: UIButton) {
        ...
        //lblCollectionCards
        tap()
        Timer.scheduledTimer(timeInterval: 1.5, target: self, selector: #selector(Lv2ViewController.finishGame), userInfo: nil, repeats: false)
        Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(Lv2ViewController.updateValue), userInfo: nil, repeats: false)
    }
    ...
    @IBAction func btnANext(_ sender: UIButton) {
        let nextScr = storyboard?.instantiateViewController(withIdentifier: "Lv3ViewController")
